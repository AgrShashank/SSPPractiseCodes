...
public class PersonalClientPage extends LoadableComponent<HomePage> {
	 ...
	 *            : Webdriver
	 */
	public PersonalClientPage(WebDriver driver, ExtentTest report) {
		this.driver = driver;
	...
	protected void isLoaded() {
		...
		if (!driver.getTitle().contains("Personal Client Details")) {
			Log.fail("Personal Client Details Page did not open up. Site might be down.", driver, extentedReport);
		} else {
			Log.message("Successfully navigated to Client/Policy Search Page", driver, extentedReport);
		}
	}
