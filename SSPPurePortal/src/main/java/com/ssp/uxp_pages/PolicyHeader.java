...
public class PolicyHeader extends LoadableComponent<HomePage> {
	 ...
	 *            : Webdriver
	 */
	public PolicyHeader(WebDriver driver, ExtentTest report) {
		this.driver = driver;
	...
	protected void isLoaded() {
		...
		if (!driver.getTitle().contains("SSP - Pure Insurance")) {
			Log.fail("Policy header Page did not open up. Site might be down.", driver, extentedReport);
		} else {
			Log.message("Successfully navigated to Policy header Page", driver, extentedReport);
		}
	}
