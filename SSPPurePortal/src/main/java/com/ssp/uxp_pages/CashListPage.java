...
public class CashListPage extends LoadableComponent<HomePage> {
	 ...
	 *            : Webdriver
	 */
	public CashListPage(WebDriver driver, ExtentTest report) {
		this.driver = driver;
	...
	protected void isLoaded() {
		...
		if (!driver.getTitle().contains("Pay Now")) {
			Log.fail("Cash list Page did not open up. Site might be down.", driver, extentedReport);
		} else {
			Log.message("Successfully navigated to Cash list Page", driver, extentedReport);
		}
	}
