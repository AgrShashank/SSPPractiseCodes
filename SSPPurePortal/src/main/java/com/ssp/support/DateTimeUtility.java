...
public class DateTimeUtility {
	...
	/**
	 * Method gets the current time from the web clock and convert to mmss
	 * format
	 * 
	 * @return time - web clock time
	 */
	public static String getTimeIn_MMSS() {
	...
	/**
	 * Returns the list of installment dates calculated based on given
	 * parameters.
	 * 
	 * @param preferredDay
	 * @param totalInstallments
	 * @return the installment dates in <code>List</code> representation, or
	 *         null if no <code>List</code> representation is applicable.
	 */
	public static List<String> generateInstallmentDates(int preferredDay, int totalInstallments) {
		List<String> lstInstDate = new ArrayList<String>();
		String cDate = getCurrentDate();
		int currentDay = Integer.parseInt(cDate.split("/")[0]);
		int nextInstallmentDay = preferredDay;
		int nextInstallmentMonth = Integer.parseInt(cDate.split("/")[1]);
		int nextInstallmentYear = Integer.parseInt(cDate.split("/")[2]);

		for (int i = 1; i <= totalInstallments; i++) {
			if (i > 1 || (i == 1 && preferredDay < currentDay)) {
				nextInstallmentMonth = (++nextInstallmentMonth % 12 == 0) ? 12 : nextInstallmentMonth % 12;
				if (nextInstallmentMonth == 1)
					nextInstallmentYear++;
			}
			int daysInMonth = YearMonth.of(nextInstallmentYear, nextInstallmentMonth).lengthOfMonth();
			nextInstallmentDay = (preferredDay > daysInMonth) ? daysInMonth : preferredDay;
			lstInstDate.add(String.format("%02d", nextInstallmentDay) + "/"
					+ String.format("%02d", nextInstallmentMonth) + "/" + nextInstallmentYear);
		}
		return lstInstDate;
	}
