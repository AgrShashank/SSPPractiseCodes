...
public class Log extends BaseTest {
	...
	public static void testCaseResult(ExtentTest extentedReport) {
		...
		if (reporterOutput.contains("FAILSOFT")) {
			...
			fail("Test Failed", extentedReport);
		} else if (reporterOutput.contains("failed")) {
			fail("Test Failed", extentedReport);
		} else if (reporterOutput.contains(FAIL_HTML_BEGIN)) {
			fail("Test Failed", extentedReport);
		} else {
	...
	/**
	 * message print the test case security test message in the log with
	...
	public static void softAssertThat(boolean status, String passmsg, String failmsg, WebDriver driver,
		...
		if (finalDecision) {
			...
			/*
			 * getScreenShotHyperLink(screenShotFolderPath + inputFile);
			 * Reporter.log(PASS_HTML_BEGIN + passmsg + "&emsp;" +
			 * getScreenShotHyperLink(inputFile) + PASS_HTML_END1 +
			 * PASS_HTML_END2);
			 */
		} else {
			/*
			 * Reporter.log(FAIL_HTML_BEGIN + failmsg + "&emsp;" +
			 * FAIL_HTML_END1 + FAIL_HTML_END2);
			 * 
		...
		if (!status) {
			...
			if (finalDecision) {
				...
				extentedReport.log(LogStatus.FAIL, failmsg + "&emsp;");
			}
		} else {
			Log.message(passmsg, driver, takeScreenShot);
			...
			if (finalDecision) {
				...
				extentedReport.log(LogStatus.PASS, passmsg + "&emsp;");
			}
